...
Everything the form previously asked for beyond this — career stage, availability, interest areas, why-join, expertise to share, topics to learn, and a professional reference's name/contact — is **no longer collected at application time**. The Board now approves or rejects on identity alone; the applicant fills in the rest of these fields themselves on their Profile (§4.3) after their first sign-in, via a one-time redirect to `/profile` (see §4.3). The Charter's "at least one professional reference" requirement for the **Open Applications** phase (§3.2, phase 3) is consequently **not enforced as an application gate today** — flagged as an open question, §11.
...
Since `/join` no longer collects career stage, expertise, why-join, or a professional reference (§3.1), the review UI at `/admin/applications/[id]` shows only identity fields (name, email, title, LinkedIn, country, requested tier, how-heard) — there is no longer a detailed vetting dossier to review before approving.
...
- **First-sign-in profile completion, soft nudge:** since `/join` only collects identity-minimum fields (§3.1), a newly-approved member lands directly on the What's New feed (`/whats-new`) on their very first sign-in (there is no separate welcome splash; the first sign-in still triggers the existing welcome shout-out, §4.10) and is nudged to finish their Profile (career stage, country/region, title/specialty, availability, and expertise/topics-to-learn). Unlike the original design, this is not a hard gate: the member can browse and use every other member page immediately, with a persistent (non-dismissible-until-complete) banner naming the still-missing fields and linking to `/profile` shown on every page until they're all saved — replaced an earlier hard-redirect version after new members reported it as too strict a barrier to browsing before finishing their profile. The nudge applies only to members whose account was created after the original `/join` field-reduction shipped (`User.requiresProfileOnboarding`) — members who joined under the old full-application flow are grandfathered and never nudged on profile completeness, since their `Profile` rows were never backfilled with that historical data.
...
- **System-generated, still confirmed (peer or admin, depending on counterpart) (§11's resolved open question #12):** two triggers auto-*create* an `earned` entry without the member filling out the "Log Contribution" form, but neither is confirmed by construction — both enter `pending` and follow the same confirmation rules as any self-reported entry (the counterpart vs. no-counterpart split, below):
  - **Meeting request, peer-confirmed:** accepting a meeting request auto-creates an `earned` entry for the recipient (the member who actually gave the consultation, e.g. Knowledge discussion, 0.5 hr), naming the requester as counterpart — but unlike the requester's spend above, this entry is **not** confirmed by construction. It follows the counterpart-confirmation path: the requester (already a party to the meeting, already spending their own hours) must confirm it from `/contributions` before it counts toward the recipient's balance. This is deliberate, not an oversight — it's an anti-fraud check so the recipient can't unilaterally credit themselves for a meeting just by accepting it. The recipient never has to manually submit anything; only the confirmation step is manual, and it falls to the requester. **Exception:** if the recipient is themselves an admin, they may confirm their own entry (see admin self-confirmation note below).
  - **Blog post publish, admin-confirmed (§4.8):** publishing a blog post auto-creates an `earned` entry for the author (Write a blog post, 0.5 hr) — but unlike the meeting-request case, it names **no counterpart**; there's no other member party to the act of publishing. Per the no-counterpart rule in the self-reported bucket below, that means it follows the **admin**-confirmation path, not peer-confirmation: it requires confirmation from `/admin/ledger` before it counts toward the author's balance. The author never has to manually submit anything; only the confirmation step is manual, and it falls to an admin rather than a peer (or to the author themselves, if the author is an admin).
- **Self-reported, then confirmed:** everything without a system-of-record trigger (curating a resource, an ad-hoc knowledge discussion not tied to a formal Event, administrative volunteer work, etc.) is submitted by the member via the "Log Contribution" form (activity type, optional counterpart, optional note) and enters as `pending`.
  ...
  - **Admin self-confirmation:** admins are a trusted role, so unlike non-admin submitters they *may* confirm or reject their own pending entries (any type, counterpart or not) — every resolution still records `resolvedByUserId` and (for admin rejections) a required reason, so self-resolution stays part of the immutable audit trail rather than bypassing it. Non-admin members can never resolve their own entries.
...
- **Audience-restricted events (new):** a host can restrict an event to a named invite list instead of the whole community (`Event.visibility`: `community` | `invited`). A restricted event, and everything about it, is visible only to its host and its invited members — it's excluded from the public `/events` listing and the community calendar, can't also be flagged `open`, and requires at least one invitee.
- **Discussion thread (new):** any member who can see the event (any member for a community event; the host or an invitee for a restricted one) can start a discussion thread for it on demand from its detail page via a "Start a Discussion" button — mirroring the Knowledge Library's on-demand thread (§4.9) rather than a create-time option, so a thread can be added at any point in an event's life, not only at submission. It posts a linked thread (titled after the event, opening post linking back to it) into the Discussion Forums' Events category (§4.13). Available for a restricted event exactly as for a community event — see §4.13 for how the thread's own visibility follows the event's restriction rather than being blocked outright.
...
- **Anonymous registration for open events (new):** an `open` event's card on the public `/events` listing shows a "Register" action to a signed-out visitor (distinct from the members-only "Join to RSVP" CTA) that captures name/email into a new `EventRegistration` record — no account required. This exists to (a) let the public actually register interest in a free/open event, and (b) build a list of non-members who've engaged with an open event, for later membership-campaign outreach. A signed-in member sees the existing RSVP toggle on an open event's card instead (previously this CTA was missing entirely for open events). Registering is unrelated to `RSVP`/`Attendance`/Knowledge Hours — it never creates either. The confirmation email sent on registration includes the event's meeting/video link (when set) — registering is the visitor's deliberate signal of intent to attend, so there's no reason to withhold it past that point — plus a short membership pitch linking to `/join`, in service of (b) above.
- **Recurring events (new):** submitting or editing an event, a host can set a repeat schedule — daily, weekly, or monthly, an interval ("every N weeks"), a weekday selection for weekly, and an optional end date — via `EventRecurrence` (one row per recurring series, attached to the master `Event`). Individual occurrences aren't stored as separate `Event` rows; they're expanded from the rule at query time, bounded to a rolling window per surface (member calendar, upcoming list, public `/events` listing, dashboard widget), so a never-ending series doesn't need any background job to keep generating rows. RSVP and cancellation apply to the whole series — there's no way in v1 to RSVP to or cancel a single occurrence, or to edit/skip just one date (see §11's open question). The event's underlying Google Calendar entry syncs as a native recurring event (an `RRULE`), not one Calendar event per occurrence, and the `.ics` "Add to calendar" download carries the same `RRULE` so a member's own calendar app expands the series correctly. Recording a host's `Attendance` is per-occurrence (see the next bullet), so a recurring host earns Knowledge Hours once per session actually hosted, not once for the whole series.
- Entities: `Event` (add `meetingUrl`, `deidentificationConfirmed`), `EventRecurrence` (`eventId` unique, `frequency`: daily/weekly/monthly, `interval` ≥ 1, `byWeekday` — 0–6, weekly only — `until`, nullable), `RSVP`, `Attendance` (add `occurrenceDate` — the specific session an attendance row is for, equal to the event's own `startsAt` for a non-recurring event; unique per `(eventId, userId, occurrenceDate)`, not just `(eventId, userId)`, so a recurring host's Knowledge Hours auto-earn can post once per session), `EventRegistration` (new — `eventId`, `email`, `name?`, `createdAt`, unique per `(eventId, email)`; no `userId`, mirroring the nullable-account-capture pattern used by `Donation`/`MembershipApplication`).
...
Blog has been consolidated into §4.9 Knowledge Library as the `blog_post` content type — see below. This includes a real behavior change: blog posts now go through the Library Steward pre-publish review workflow instead of publishing instantly, and are member-only rather than publicly readable (§5, §11).

### 4.9 Knowledge Library

- Searchable, filterable resource repository: recorded lectures, articles/summaries, case studies, guidelines, and **member-written blog posts** (the `blog_post` content type, folded in from the former standalone Blog domain).
- Filters: content type, specialty, career-stage level (Student-Friendly / Early Career / Advanced / All Levels).
- Card metadata: contributor, category/specialty, file type icon, upload date, description. A `blog_post` card shows the same fields, with its excerpt auto-derived from the post body rather than a separately-typed description.
- "Submit Resource" action for members — for `blog_post`, this is the "Write a Post" experience: a Tiptap rich-text editor (per system-design.md) for the body, no attachment/external link/YouTube URL fields.
- **Library Steward review workflow:** submitted resources do **not** publish immediately — they enter `pending_review`. A **Library Steward** (a volunteer member holding the `moderator` role, §2.1, ideally one per specialty area at scale) reviews for quality, correct tagging, and disclaimer/de-identification compliance before publishing. Only `published` items appear in search/browse; `pending_review` items are visible to their submitter (with status) and to Stewards/admins on a review queue. This now applies uniformly to `blog_post` too — the former Blog domain's instant-publish-on-write behavior is retired.
- **Community flagging:** any member can flag a published item as inaccurate or outdated. Flagged items are reviewed by a Steward and, if needed, escalated to the Board — flagged items stay visible (not auto-hidden) unless a Steward removes them, consistent with Knowledge_Library.md's "community flagging, not formal peer review" quality model.
- **De-identification confirmation:** submitting a **case study** type resource requires the same explicit confirmation checkbox as case-discussion events (§4.6) — "I confirm all patient information has been de-identified" — before it can enter review.
- **Content licensing consent:** every content type, `blog_post` included, requires a one-time/per-submission IP consent — the contributor retains ownership, and grants Nasiha a non-exclusive right to display it to the membership (see §4.15).
- Preview: PDF.js for document types (per system-design.md). **Recorded lectures:** hosted on Nasiha's own YouTube channel (unlisted or public, per org preference), not stored as binaries — a submitted "recorded lecture" resource is a YouTube URL/video ID plus the same metadata (contributor, specialty, description, de-identification/licensing consent) as any other library item, previewed via a standard YouTube embed rather than a custom player. This mirrors the Events domain's existing pattern (§4.6) of Nasiha hosting the *listing*, not the media itself — Zoom/Meet for live sessions, YouTube for recordings. A `blog_post` item's body renders inline instead, the same rich-text-to-HTML rendering the old Blog domain used.
- **Editing a submission:** the contributor, a Library Steward, or an admin can edit a submission's title/description (or body, for `blog_post`)/content type/level/category/tags/attachment (or YouTube URL) from `/library/[id]/edit`. Stewards get edit rights here, not just publish/reject, since fixing tagging/quality issues directly (their §4.9 review job) is faster than reject-and-ask-to-resubmit. Editing a `published`/`flagged` item goes live immediately with no re-review — only the *initial* submission gates on Steward review. Editing a `rejected` item is the resubmit path (there's no separate "resubmit" action) and moves it back to `pending_review`.
- **Detail page:** `/library/[id]` — a real, deep-linkable page showing title, description, category/level/content-type/tag badges, contributor byline (linked to their Directory profile when listed), the inline PDF.js/YouTube/body preview, and Edit/Flag actions. `pending_review`/`rejected` items 404 here, same gate as browse — a contributor previews their own unpublished submission via `/library/[id]/edit` instead.
- **Discussing a resource:** discussion happens in Forums, not native comments — a seeded **"Library Discussions"** forum, distinct from Research & Resources. Same on-demand pattern as an Event's discussion thread (§4.6/§4.13): a Library item's thread (`ForumThread.knowledgeItemId`) is created the first time any member clicks the detail page's "Start a Discussion" button — not at submission time — with an auto-authored opening post linking back to the resource. Once a thread exists, the button is replaced by the thread itself, rendered inline on the same page. `blog_post` discussion works identically, including Forums' `@Full Name` mention convention (§4.13/§4.10) — the former Blog domain's native `PostComment` model and its separate mention-notification path are retired; existing blog comments were migrated into this forum-thread shape.
- **Knowledge Hours credit:** publishing any submission (via Steward review, `blog_post` included) posts a pending "Curate a resource" earn transaction for the contributor (0.5 hr, §4.4) — no manual "Log Contribution" step required. The entry has no counterpart, so per §4.4's no-counterpart rule it requires **admin** confirmation from `/admin/ledger` before it counts toward the contributor's balance. (The former Blog-specific "Write a blog post" rule/rate is retired — same 0.5 hr amount, now earned at publish time via review rather than at write time.)
- Entities: `KnowledgeItem` (`status`: `pending_review` / `published` / `flagged` / `rejected`, `deidentificationConfirmed`, `licenseConsented`, `body` — rich text, `blog_post` only; recorded-lecture items store `youtubeUrl` instead of a `KnowledgeAttachment`), `KnowledgeCategory`, `KnowledgeTag`, `KnowledgeAttachment` (documents/articles/case studies only — not video), `LegacyBlogSlug` (redirect-only lookup for old `/blog/[slug]` URLs).
- Storage: MinIO (document/article binaries only — no video) + PostgreSQL (metadata) + Meilisearch (search index — index only `published`/`flagged` items).
- Routes: `GET /api/admin/library/review-queue`, `POST /api/library/:id/flag`, `POST /api/admin/library/:id/publish`, `PATCH /api/library/:id` (editing), `POST /api/library/:id/discussion` (start/resolve the discussion thread) alongside the base library routes.
- Routes: `/library`, `/library/[id]` (detail page), `/library/[id]/edit` (contributor, Steward, or admin only), `/library/new` (member auth only). All member-only — unlike the former public `/blog`, there is no signed-out-readable route for Library content, `blog_post` included (§5, §11). Old `/blog`, `/blog/[slug]`, `/blog/new`, `/blog/[slug]/edit` URLs 308-redirect to their Library equivalents.
...
- Types: new inbox message, meeting request (received / accepted / declined / rescheduled), event reminder, membership update (application approved/rejected), blog comment, forum reply, **mention** (a member explicitly tagged via `@Full Name` in a forum post or blog comment, §4.8/§4.13 — distinct from the generic blog-comment/forum-reply notification; a recipient gets only the mention notification when both would otherwise fire for the same post), contribution awarded, resource review update (a submitter's item published/rejected). Digest and Board Announcement (below) are also first-class types, delivered on their own batching/broadcast rules rather than per-item.
...
- **"Mark all read" vs. "clear read" (new):** two distinct bell actions. "Mark all read" only sets `readAt` on every unread notification — the rows stay. "Clear read" hard-deletes every already-read notification (`readAt` not null) for that member, pruning the bell's list and freeing the rows; it never touches unread notifications, so nothing is deleted before the member has actually seen it.
...
**Structure:** topic-based forums, seeded with six categories from the source doc plus one added later:
...
Two more forums are seeded alongside these seven — **Events Discussion** (§4.6) and **Library Discussions** (§4.9) — but don't appear in the `/forums` category index: see the norms below for why.

**Norms (enforced, not just documented):**
- All posts are visible to the full membership (no private/DM-style forums in v1) — **with two exceptions:** an inherited one (below) and a member-initiated one (next bullet). The inherited exception: a thread linked to an audience-restricted event (§4.6) or a restricted Knowledge Library item (§4.9) is visible only to that event's/item's host or contributor and invited members, not the whole community. This is thread-level, not forum-level — a thread linked to a community (non-restricted) event or item is fully public as normal. The restriction applies everywhere the thread could surface: the category's browse/search list, the thread page itself (a non-invitee's direct link 404s, rather than 403ing and confirming the thread exists), replying, the view counter, and the thread's appearance on its author's Directory profile. **Events Discussion and Library Discussions are excluded from the `/forums` category index (new):** a thread in either is only ever created on demand from its linked event's/item's own detail page, never by a member browsing categories on `/forums` and starting a new thread there (there's no such affordance for these two), so listing them as a browsable index card is a dead end at best — and for a thread inherited-restricted per the paragraph above, would advertise a private thread's existence to members who can't open it. Their threads stay fully reachable via the source event's/item's detail page and by direct `/forums/events/[threadId]` or `/forums/library-discussions/[threadId]` URL, still subject to every visibility check above; only the `/forums` index listing itself hides the two categories. Every invited member — not only those who've already posted — gets the normal reply notification (§4.10) for new activity on the thread, since being invited to the event/item is what puts them "in" the conversation, not having posted first.
- **Member-initiated restricted threads (new):** independent of the inherited exception above, any member starting a **standalone** thread — one with no linked `eventId`/`knowledgeItemId` — can restrict it to a named invite list instead of posting to the whole community, the same `ForumThread.visibility`: `community` | `invited` pattern as Audience-restricted events (§4.6), applied directly rather than derived from a parent Event/Knowledge item. The composer's audience picker draws from the same Directory-eligible member set as the Inbox/Directory pickers (§4.5/§4.7); the thread requires at least one invitee to go `invited`, can't also be admin-`pinned`, and defaults to `community` if the author leaves it alone. Once a thread is linked to an Event or Knowledge item it can no longer carry its own `visibility`/invitees — a thread is restricted by exactly one mechanism, inherited or member-initiated, never both. **The invite list is editable post-creation (resolved, §11.16):** the author, or an admin, can add or remove invitees from the thread page at any time — same "author or admin" edit authority as Blog (§4.8) and Library (§4.9). Removing an invitee revokes their access immediately (the thread 404s for them on their very next request, same as a non-invitee who was never invited); adding one grants access immediately and fires a dedicated notification pointing them to the thread — the per-viewer feed filter below is evaluated live, so the thread does become reachable in their What's New feed from that point on, but purely at its original `createdAt` position (adding an invitee never bumps it), so without the notification a newly added invitee would have no reliable way to notice it short of paging back through the feed to its original date. A restricted thread must always keep at least one invitee — removing the last one is rejected, mirroring the "requires at least one invitee" rule at creation; there is no path to flip an already-`invited` thread back to `community` in v1. Everywhere the thread can surface, and the reply-notification behavior for every invitee, matches the inherited exception above (browse/search list, direct 404 vs. 403, view counter, Directory-profile appearance, notify-whether-or-not-posted) — including that a moderator/admin can still see and act on a restricted thread through the shared moderation queue (Moderation, below) regardless of its `visibility`, since moderation authority isn't gated by audience. `@`-tagging (below) a member who isn't an invitee of a restricted thread is rejected by the composer's autocomplete rather than silently sent — a mention notification linking to a thread the recipient can't open would otherwise leak the thread's existence to a non-invitee. This "everywhere the thread can surface" guarantee also covers the app's What's New feed (dashboard/`/whats-new`, an already-built cross-domain activity feed not yet otherwise documented in this PRD): today that feed's standalone-thread query applies no per-viewer visibility check at all — safe only because standalone threads currently have no visibility concept — so shipping `ForumThread.visibility` requires adding the same per-viewer filter there that the feed already applies to restricted events and restricted Library items, or a member-initiated restricted thread's title/excerpt would otherwise leak to every member's feed on creation. **Feed vs. Notification, for replies specifically:** the What's New feed only ever surfaces a thread once, at creation (`orderBy: createdAt`, no "last activity"/"last reply" field to bump) — a new reply never creates a fresh feed entry or re-surfaces an existing one, for any thread, restricted or not. So an invitee's awareness of new replies on a restricted thread comes entirely from the reply notification (§4.10, "notify-whether-or-not-posted" above), never from the feed.
...
- Reply notifications (§4.10) already reach only a thread's participants (its author and anyone who has posted in it) plus any `@`-tagged member — not the whole forum's membership — so browsing or posting in a category doesn't produce inbox overload ("notification overload," the anti-pattern the source doc calls out) on its own. There is no separate per-forum follow/mute mechanism in v1; the general community Digest (§4.10) remains available independent of any per-forum opt-in.
...
- **User tagging (new):** a member can tag another member in a post/reply body by typing `@Full Name` (matched against Directory-eligible members, §4.5 — no separate username/handle field). The composer offers an autocomplete suggestion list once `@` + at least one character is typed. A tagged member gets a dedicated **mention** notification (§4.10), distinct from the generic reply notification other thread participants receive — if a tagged member would also receive the generic notification for the same post, only the mention notification is sent, to avoid double-notifying.

**Entities:** `Forum` (category), `ForumThread` (title, author, forum, created date, pinned flag, add `visibility`: `community` | `invited`, default `community` — new, mirroring `Event.visibility` §4.6), `ForumThread` invite list (new — mirrors the Event/Knowledge item invite list, §4.6/§4.9; one row per invited member, only populated when `visibility` is `invited`), `ForumPost` (thread, author, body, created date — top-level posts and replies are the same entity, threaded by `parentPostId`).

**Moderation:** forum moderation uses the existing `moderator` role (§2.1); flagged posts route the same way as flagged Library content (§4.9) — one shared moderation queue, not a separate one per domain. Moderators/admins can open a restricted thread regardless of `visibility`/invite list — the moderation queue's own access check, not thread-visibility, gates their view.

**Routes:** `GET /api/forums`, `GET /api/forums/:forumId/threads`, `POST /api/forums/:forumId/threads` (add optional `visibility` + invitee list, new), `PATCH /api/forums/threads/:threadId/invitees` (new — author or admin only; add/remove invitees on an `invited`-visibility thread, rejects dropping the last invitee), `POST /api/forums/threads/:threadId/posts`, `POST /api/forums/posts/:postId/flag`.
...
/dashboard           Stats row (hours balance, lifetime earned, sessions contributed, total member count —
                     ...
                     recently-added library widget (includes recent blog posts, the blog_post content type)
...
/admin/users
/admin/applications
/admin/content         Shared flagged-content queue: Library (blog_post included), Forums (§4.11)
/admin/events
/admin/ledger
/admin/team
...
- `event_recurrences` (§4.6, recurring events) — one row per recurring master, `event_id` unique: `frequency` (daily/weekly/monthly), `interval` (≥ 1), `by_weekday` (0–6, weekly only), `until` (nullable). No occurrence-instance table — a recurring event's individual sessions are expanded from this rule at query time, bounded to a rolling window per surface, rather than materialized as rows; this keeps a never-ending series (`until` null) from requiring a background job to keep generating future rows, at the cost of the member calendar/upcoming list/public listing each only showing occurrences within their own bounded window rather than the full unbounded series.
- `attendance` gains an `occurrence_date` column and its uniqueness changes from `(event_id, user_id)` to `(event_id, user_id, occurrence_date)` (§4.6) — without this, a recurring event's host could only ever earn the auto-posted Knowledge Hours transaction once, total, for the entire series (one `Event` row means one attendance row under the old 2-column key), instead of once per session actually hosted.
...
Indexed domains: `profiles`, `knowledge_items` (includes the `blog_post` content type, folded in from the former separate `posts` index), `events`. Sync strategy: DB write → queue event (BullMQ) → worker updates index.
...
- **Content moderation:** library submissions (blog posts included, as the `blog_post` content type), forum posts, and profile content need an admin moderation path (flag/remove) — for Forums this is a hard requirement (§4.13, "commercial promotion... needs actual moderation tooling"), not just implied. For Library it's implied by system-design.md's Admin Domain "content moderation" feature. This should extend to uploaded profile photos (flaggable/removable by an admin) since it's user-uploaded media.
...
| **5** | Knowledge Library (with Steward review queue, including the `blog_post` content type — Blog shipped initially as its own standalone domain in this phase, then was consolidated into the Library post-launch, §4.8/§4.9), Discussion Forums — grouped together since both share a submit → review/moderate → publish → index pattern | Full content ecosystem: write a post or submit a library resource through one unified flow, post in a forum, find any of it via section-scoped search. **Dashboard's recently-added-library widget (including recent blog posts) goes live.** | Tiptap (rich-text editor, reused for the `blog_post` content type); PDF.js (library document preview); YouTube embed for recorded lectures (no video storage/player infra needed); MinIO `documents/`/`attachments/` buckets; Meilisearch index extended to knowledge_items (blog_post included), forum content | Full UI exists for Blog and Library at initial launch; write/submission flows are stubbed toasts. Forums have no prototype UI at all — net-new, built on the same submit/moderate pattern as Library |
...
12. ~~**Blog post editing:** §4.8 originally specified write-once-and-publish with no edit path — `Post.publishedAt` is nullable in the data model (§7) as a "draft" state, but no code path ever produces or exposes it, and no UI lets an author revise a post after publishing. Needs a decision: can a published post be edited at all, and if so by whom (author only, or admin too), does an edit reset `publishedAt` or leave the original publish date, and is there any edit-window cutoff or audit trail?~~ **Resolved:** yes — the author or an admin can edit a published post's title/body/category/tags/hero image from `/blog/[slug]/edit` (§4.8). `publishedAt` and the slug are left untouched by an edit (only `updatedAt` moves, giving a last-edited signal without a full audit trail), and there's no time-boxed edit window in v1 — this mirrors the existing "admin has the same moderation authority as the author" precedent already established by `resolvePostFlag`'s takedown path.
14. **Professional reference requirement no longer enforced at application time:** the Charter requires "at least one professional reference" once the **Open Applications** phase begins (§3.2, phase 3). As of the `/join` field-reduction (§3.1), the application form no longer collects a professional reference at all — the Board approves on identity alone (name/email/title/country/how-heard), and reference-style vetting, if wanted, would have to happen out-of-band or be reintroduced elsewhere (e.g. requested during profile completion, or as a manual Board step before Open Applications launches). Needs a decision before the Open Applications phase goes live: reinstate a reference field somewhere in the flow, or accept identity-only admission as the permanent policy and update the Charter/Risk_and_Liability.md accordingly.
15. ~~**No auto-earn for the consulted party on an accepted meeting request:** §4.4's hybrid posting model defines exactly one auto-post trigger for meeting acceptance — a `spent` transaction for the *requester* (Expert Consultation, 1.0 hr). It defines no corresponding `earned` transaction for the *recipient* (the member who actually gave the consultation), so as built, the person providing their time gets no automatic credit at all. Today the only path to any credit is for the recipient to separately self-report via "Log Contribution" (e.g. "Knowledge discussion," 0.5 hr) naming the requester as counterpart for peer confirmation — a manual, disconnected step nobody is prompted to take, at a different activity/hour rate than what the requester spent. Needs a decision: add a symmetric auto-earn for the recipient at meeting acceptance (and if so, at what rate — mirroring the requester's 1.0 hr Expert Consultation spend, or a distinct rate), or accept the current asymmetric spend-only behavior as a documented v1 limitation.~~ **Resolved:** accepting a meeting request now auto-*creates* a `pending` Knowledge discussion (0.5 hr) earn entry for the recipient, naming the requester as counterpart — but it is **not** auto-confirmed like the requester's spend side. It still requires the requester's peer confirmation from `/contributions` before counting toward the recipient's balance. This asymmetry (spend auto-confirms, earn doesn't) is deliberate: it's an anti-fraud check so the recipient can't unilaterally credit themselves for a meeting just by accepting it, while still sparing the recipient any manual submission step (§4.4).
16. ~~**Editing a restricted forum thread's invite list:** §4.13's new member-initiated restricted threads let an author name invitees only at creation time — there's no specified route/UI for adding or removing an invitee afterward, unlike (for example) editing a Library submission's tags post-publish (§4.9). Needs a decision: accept invite-list-fixed-at-creation as a documented v1 limitation, or add a `PATCH` path (author, or author + admin) to adjust it later.~~ **Resolved:** yes, editable post-creation — the thread's author or an admin can add or remove invitees at any time from the thread page (§4.13), same "author or admin" authority precedent as editing a Blog post (§4.8) or Library submission (§4.9).
...
- [ ] A visitor can browse Home/About/Our Team/How-to-Join/Events/Contact without an account. (Blog is no longer a public section — it's the member-only `blog_post` Library content type, §4.8/§4.9/§11.18.)
...
- [ ] A visitor can submit a membership application collecting only identity-minimum fields (name, email, country, optional title/LinkedIn/requested tier, how-heard) — no career stage, expertise, why-join, or professional reference collected up front (§3.1); an admin can review, approve (assigning a tier), or reject it with notes, from a real `/admin/applications` UI.
- [ ] An approved applicant receives credentials/invite and can log in via real auth (no demo-login shortcuts in production).
- [ ] A member created after the /join field-reduction lands on the What's New feed (`/whats-new`) on first sign-in, and can browse and use every other member page immediately, with a persistent banner naming the still-missing Profile fields (career stage, country/region, title/specialty, availability, expertise/topics-to-learn) and linking to `/profile` until all are filled in. Members who joined before this change are grandfathered and never nudged (§4.3).
- [ ] A logged-in member sees a Dashboard (Hours-balance stats row, upcoming-events widget, recently-added-library widget — includes recent blog posts, total-member count — not a live "members online" presence indicator) and can reach Settings (notification/digest preferences, password change, data export/deletion request); both exist from Phase 1 onward and are fully populated by Phase 6 (§10).
...
- [x] From the Inbox page itself, a member can open a "Message" or "Request Meeting" picker, search for a Directory-eligible member by name, and send them a message or meeting request without visiting the Directory page first (§4.7).
...
- [ ] A host can set a repeat schedule (daily/weekly/monthly, interval, weekday selection, optional end date) when submitting or editing an event; the series' occurrences appear on the correct dates in the member calendar month grid, the upcoming list, and the public `/events` listing, each within its own bounded window. RSVPing to, or cancelling, any occurrence applies to the whole series (§4.6; no single-occurrence exceptions in v1, per §11 item 17).
- [ ] A recurring event's underlying Google Calendar entry is a native recurring event (an `RRULE`), not one Calendar event per occurrence; the `.ics` "Add to calendar" download carries the same `RRULE`. A host recording attendance for two different past occurrences of the same recurring event produces two separate Knowledge Hours auto-earn transactions, not one (§4.6).
- [ ] A member can write and submit a blog post (the `blog_post` content type, a Tiptap body instead of a file/link) to the Knowledge Library; it enters `pending_review` like any other submission, and other members can read it at `/library/[id]` once a Steward publishes it — it is member-only, not publicly readable (§4.8/§4.9/§11.18).
- [ ] A member can submit any Library resource — a file upload (MinIO-backed), an external link, a YouTube URL, or a written blog post — with metadata; it enters `pending_review` and only appears in search/browse after a Library Steward publishes it from `/admin/library/review-queue`, which auto-posts a pending "Curate a resource" earn transaction (0.5 hr) for the contributor with no counterpart, requiring admin confirmation from `/admin/ledger` before it counts toward the balance (§4.4/§4.9).
- [ ] The submission's contributor, a Library Steward, or an admin can edit it from `/library/[id]/edit`; editing a `published`/`flagged` item goes live immediately with no re-review, and editing a `rejected` item resubmits it to `pending_review` (§4.9, §11.12).
- [ ] A previously-published old `/blog/[slug]` URL 308-redirects to the migrated post's `/library/[id]` (§11.18).
- [ ] A member can view a Library resource's own detail page at `/library/[id]` (published/flagged only) and, from there, either start or join a discussion about it in the seeded "Library Discussions" forum — the thread is created on demand, on the first "Start a Discussion" click, not at submission time (§4.9).
- [ ] A member can browse and post in Discussion Forums (the seven categories listed on `/forums` — Events Discussion and Library Discussions are seeded but excluded from that index, reachable only from their source event's/item's page or a direct thread URL); posts are visible to the full membership, and reply notifications reach only a thread's participants and any `@`-tagged member — not the whole forum — so no separate per-forum follow/digest mechanism is needed.
...
- [ ] Publishing any Library item, blog posts included, requires an explicit content-licensing consent step.
- [ ] An RSVP'd member, and a signed-out visitor who registers for an `open` event (via its confirmation email), sees the event's meeting/video link; it is never shown on the public `/events` listing itself.
