...
- **Knowledge Hours credit (new):** publishing a post automatically posts a pending Knowledge Hours earn transaction for the author (Write a blog post, 0.5 hr, §4.4) — no manual "Log Contribution" step required. The entry has no counterpart (publishing isn't a peer-to-peer act), so per §4.4's no-counterpart rule it requires **admin** confirmation from `/admin/ledger` before it counts toward the author's balance.
- Comments supported per data model (not shown in prototype UI but required per system-design.md).
- **User tagging (new):** same `@Full Name` mention convention as Discussion Forums (§4.13) — a commenter can tag another Directory-eligible member, who receives a dedicated **mention** notification (§4.10) instead of the generic blog-comment notification.
- **Editing a published post (new, resolved §11.13):** the author, or an admin, can edit a published post's title/body/category/tags/hero image from `/blog/[slug]/edit`. The original `publishedAt` and slug are unchanged by an edit — only `updatedAt` moves — and there's no edit-window cutoff or edit-history/audit trail in v1.
- Entities: `Post`, `PostCategory`, `PostTag`, `PostComment`.
- Routes: `/blog`, `/blog/[slug]` (public-readable; write requires member auth), `/blog/[slug]/edit` (author or admin only).
...
- **Editing a submission (new):** the contributor, a Library Steward, or an admin can edit a submission's title/description/content type/level/category/tags/attachment (or YouTube URL) from `/library/[id]/edit` — Stewards get edit rights here, not just publish/reject, since fixing tagging/quality issues directly (their §4.9 review job) is faster than reject-and-ask-to-resubmit. Editing a `published`/`flagged` item goes live immediately with no re-review, mirroring Blog's "no re-review on edit" precedent (§4.8/§11.12) — only the *initial* submission gates on Steward review. Editing a `rejected` item is the resubmit path (there's no separate "resubmit" action) and moves it back to `pending_review`.
- **Detail page (new):** `/library/[id]` — a real, deep-linkable page (replacing the old preview-modal-on-the-browse-grid pattern) showing the same depth as `/blog/[slug]`: title, description, category/level/content-type/tag badges, contributor byline (linked to their Directory profile when listed), the inline PDF.js/YouTube preview, and Edit/Flag actions. `pending_review`/`rejected` items 404 here, same gate as browse — a contributor previews their own unpublished submission via `/library/[id]/edit` instead.
- **Discussing a resource (new):** rather than native comments (which Blog has via `PostComment`), a Library item's discussion happens in Forums — a new seeded **"Library Discussions"** forum, distinct from Research & Resources. Same on-demand pattern as an Event's discussion thread (§4.6/§4.13): a Library item's thread (`ForumThread.knowledgeItemId`) is created the first time any member clicks the detail page's "Start a Discussion" button — not at submission time — with an auto-authored opening post linking back to the resource. Once a thread exists, the button is replaced by the thread itself, rendered inline on the same page.
...
- Routes: add `GET /api/admin/library/review-queue`, `POST /api/library/:id/flag`, `POST /api/admin/library/:id/publish`, `PATCH /api/library/:id` (editing), `POST /api/library/:id/discussion` (start/resolve the discussion thread) alongside the existing library routes.
- Routes: `/library`, `/library/[id]` (detail page), `/library/[id]/edit` (contributor, Steward, or admin only).
...
/dashboard           Stats row (hours balance, lifetime earned, sessions contributed, total member count —
                     ...
                     recently-added library widget, recent blog posts widget
...
- [ ] The submission's contributor, a Library Steward, or an admin can edit it from `/library/[id]/edit`; editing a `published`/`flagged` item goes live immediately with no re-review, and editing a `rejected` item resubmits it to `pending_review` (§4.9).
