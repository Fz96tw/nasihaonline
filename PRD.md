...
- **System-generated, still peer-confirmed (§11's resolved open question #12):** accepting a meeting request also auto-*creates* an `earned` entry for the recipient (the member who actually gave the consultation, e.g. Knowledge discussion, 0.5 hr), naming the requester as counterpart — but unlike the requester's spend above, this entry is **not** confirmed by construction. It enters `pending` and follows the same counterpart-confirmation path as any self-reported entry: the requester (already a party to the meeting, already spending their own hours) must confirm it from `/contributions` before it counts toward the recipient's balance. This is deliberate, not an oversight — it's an anti-fraud check so the recipient can't unilaterally credit themselves for a meeting just by accepting it. The recipient never has to manually submit anything; only the confirmation step is manual, and it falls to the requester.
