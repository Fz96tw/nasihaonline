...
- **First-sign-in profile completion, hard gate (new):** since `/join` only collects identity-minimum fields (§3.1), a newly-approved member is shown a one-time "Welcome to NASIHA" splash (`/welcome`) on their very first sign-in — the same first-sign-in event that triggers the existing welcome shout-out (§4.10) — then must complete their Profile (career stage, country/region, title/specialty, availability, and expertise/topics-to-learn) before they can reach *any other* member page. Every (member) route redirects to `/profile` until those fields are saved; the redirect re-evaluates on every navigation (not just the first), so it persists across visits until completion, not a one-time nudge. This gate applies only to members whose account was created after this change shipped (`User.requiresProfileOnboarding`) — members who joined under the old full-application flow are grandfathered and never gated on profile completeness, since their `Profile` rows were never backfilled with that historical data.
...
- [ ] A member created after the /join field-reduction sees a one-time `/welcome` splash on first sign-in, then cannot reach any other member page until they've filled in the Profile fields `/join` no longer collects (career stage, country/region, title/specialty, availability, expertise/topics-to-learn) — the redirect persists across visits until complete. Members who joined before this change are grandfathered and never gated (§4.3).
