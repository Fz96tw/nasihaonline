...
- **Discussion thread (new):** at creation, a host can optionally check "create a discussion thread for this event," which posts a linked thread (titled after the event, opening post linking back to it) into the Discussion Forums' Events category (§4.13). Available for a restricted event exactly as for a community event — see §4.13 for how the thread's own visibility follows the event's restriction rather than being blocked outright.
...
- **Discussing a resource (new):** rather than native comments (which Blog has via `PostComment`), a Library item's discussion happens in Forums — a new seeded **"Library Discussions"** forum, distinct from Research & Resources. Unlike a Case Discussion event's discussion thread (§4.6/§4.13's `ForumThread.eventId`, created eagerly via an opt-in checkbox at submission time), a Library item's thread (`ForumThread.knowledgeItemId`) is created **on demand**: the detail page shows a "Start a Discussion" button when no thread exists yet; clicking it creates the thread (with an auto-authored opening post linking back to the resource) and navigates in. Once a thread exists, every visitor instead sees "Join the Discussion" (with reply count), linking straight to it.
...
- [ ] A host can create a discussion thread for a restricted (invited-only) event exactly as for a community event; the thread is visible only to the host and invited members everywhere it could surface (browse/search, direct link, reply, view count, author's profile), and every invitee gets notified of new replies whether or not they've posted (§4.6, §4.13).
