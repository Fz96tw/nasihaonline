...
- Routes: add `GET /api/admin/library/review-queue`, `POST /api/library/:id/flag`, `POST /api/admin/library/:id/publish`, `PATCH /api/library/:id` (editing) alongside the existing library routes.
- Routes: `/library`, `/library/[id]/edit` (contributor, Steward, or admin only).
