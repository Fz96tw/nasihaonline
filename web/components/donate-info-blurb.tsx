 ...
 *
 * Width is deliberately matched to DonateForm's `mx-auto max-w-xl p-8`
 * (matching horizontal padding via px-8 instead of p-8, since vertical
 * spacing is controlled by each call site) so this text's edges line up
 * with the form fields rendered directly below it.
 */
export function DonateInfoBlurb({
  showTaxDeductibleIntro = true,
}: {
  /** False on /donate, where the tax-deductible sentence lives in the hero banner instead. */
  showTaxDeductibleIntro?: boolean;
}) {
  return (
    <div className="mx-auto max-w-xl px-8 text-base leading-relaxed text-foreground">
      {showTaxDeductibleIntro && (
        <p>
      ...
      <p className={showTaxDeductibleIntro ? "mt-4" : undefined}>
