...
const STATS: { val: number | string | null; lbl: string }[] = [
...
export async function HeroSection() {
  ...
  const stats = STATS.map((stat) => (stat.val === null ? { ...stat, val: foundingMemberCount } : stat));
  ...
  return (
    <section className="relative overflow-hidden px-8 pb-20 pt-24 text-center text-primary-foreground">
      ...
      <div className="relative mx-auto max-w-[960px]">
        <p className="inline-block rounded-full border border-primary-foreground/25 bg-primary-foreground/[.15] px-4 py-[.35rem] text-xs font-semibold uppercase tracking-[.06em] text-[#e0eaff] backdrop-blur-sm [text-shadow:0_1px_6px_rgba(0,10,40,.55)]">
          A Learning Community
        </p>
        <h1 className="mb-5 mt-6 text-[2.1rem] font-extrabold leading-[1.1] tracking-[-.02em] [text-shadow:0_2px_16px_rgba(0,10,40,.55)] md:text-[3.25rem]">
          Knowledge is better when we{" "}
          <em className="text-blue-300 not-italic">share what we know</em>.
        </h1>
        <p className="mx-auto max-w-[640px] text-[1.15rem] leading-[1.7] text-primary-foreground/90 [text-shadow:0_1px_10px_rgba(0,10,40,.5)]">
        ...
        <div className="mt-14 flex flex-wrap justify-center gap-12 border-t border-primary-foreground/20 pt-8">
          {stats.map((stat) => (
            <div key={stat.lbl}>
              <div className="text-[1.75rem] font-extrabold text-blue-300">
