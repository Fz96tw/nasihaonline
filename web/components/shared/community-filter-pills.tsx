 ...
 * already filters already-loaded client data with no URL round trip. A
 * server-driven, shareable-URL consumer (e.g. Library's browse page) needs
 * community-filter-pills-nav.tsx instead: this component is "use client"
 * (its onClick handlers require it), so a Server Component parent can't
 * pass it a plain buildHref function prop — only a Server Action — the
 * same client/server boundary rule that keeps community-category-filter.tsx
 * (Link-only, no "use client") a separate component from this one rather
 * than a single dual-mode file.
 */
