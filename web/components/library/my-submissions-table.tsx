"use client";

import { useState } from "react";
import Link from "next/link";
import { CONTENT_TYPE_LABELS, STATUS_BADGE_VARIANT, STATUS_LABELS } from "@/lib/library";
import type { MySubmission } from "@/lib/library";
import { KnowledgeStatus } from "@/lib/generated/prisma/enums";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { DeleteLibraryItemButton } from "@/components/library/delete-library-item-button";

/** /library/[id] 404s for pending_review/rejected items, even for their own contributor — link those to /edit instead. */
function isViewable(status: KnowledgeStatus) {
  return status === KnowledgeStatus.published || status === KnowledgeStatus.flagged;
}

/** Shared by /library/mine and /my-posts's Library tab — a member's own submissions, at any status. */
export function MySubmissionsTable({ submissions }: { submissions: MySubmission[] }) {
  // Save as Draft initiative — a quick "Drafts" filter so an abandoned
  // draft doesn't just rely on spotting its badge in an otherwise-normal
  // list. Client-side only (the full list is already fetched) and hidden
  // entirely when there's nothing to filter, same "don't clutter members
  // who never draft" rationale as the dashboard's draft count.
  const [draftsOnly, setDraftsOnly] = useState(false);
  const hasDrafts = submissions.some((item) => item.status === KnowledgeStatus.draft);
  const visibleSubmissions =
    draftsOnly && hasDrafts ? submissions.filter((item) => item.status === KnowledgeStatus.draft) : submissions;

  return (
    <div className="flex flex-col gap-3">
      {hasDrafts && (
        <div className="flex gap-2">
          <Button type="button" size="sm" variant={!draftsOnly ? "default" : "outline"} onClick={() => setDraftsOnly(false)}>
            All
          </Button>
          <Button type="button" size="sm" variant={draftsOnly ? "default" : "outline"} onClick={() => setDraftsOnly(true)}>
            Drafts
          </Button>
        </div>
      )}
      <div className="rounded-[10px] border shadow-sm">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Title</TableHead>
              <TableHead>Type</TableHead>
              <TableHead>Category</TableHead>
              <TableHead>Status</TableHead>
              <TableHead>Submitted</TableHead>
              <TableHead />
            </TableRow>
          </TableHeader>
          <TableBody>
            {visibleSubmissions.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center text-muted-foreground">
                  {draftsOnly ? "No drafts." : "You haven't submitted any resources yet."}
                </TableCell>
              </TableRow>
            )}
            {visibleSubmissions.map((item) => (
              <TableRow key={item.id}>
                <TableCell className="font-medium">
                  {isViewable(item.status) ? (
                    <Link href={`/library/${item.id}`} className="hover:underline">
                      {item.title}
                    </Link>
                  ) : (
                    item.title
                  )}
                </TableCell>
                <TableCell className="text-muted-foreground">{CONTENT_TYPE_LABELS[item.contentType]}</TableCell>
                <TableCell className="text-muted-foreground">
                  {item.categories.map((category) => category.name).join(", ")}
                </TableCell>
                <TableCell>
                  <Badge variant={STATUS_BADGE_VARIANT[item.status]}>{STATUS_LABELS[item.status]}</Badge>
                </TableCell>
                <TableCell className="text-muted-foreground">
                  {new Date(item.createdAt).toLocaleDateString()}
                </TableCell>
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-3">
                    <Link href={`/library/${item.id}/edit`} className="text-sm text-primary hover:underline">
                      Edit
                    </Link>
                    <DeleteLibraryItemButton
                      itemId={item.id}
                      title={item.title}
                      hasEarnedHours={item.hasEarnedHours}
                    />
                  </div>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      </div>
    </div>
  );
}
