...
import type { KnowledgeItemRosterMember } from "@/lib/library";
import type { DirectoryMember } from "@/lib/members";

/**
 * Contributor/Steward/admin-facing invited-list editor on a restricted
 * Knowledge Library item's detail page (Restricted Knowledge Library
 * Submissions, Objective 05) — add and remove invitees after submission,
 * each action posted immediately (no separate "save" step). Mirrors
 * components/calendar/manage-invitees.tsx, minus the RSVP status badge:
 * there's no RSVP analog for the Library, so invitees are shown flat.
 */
export function ManageLibraryInvitees({
  itemId,
  initialRoster,
}: {
  itemId: string;
  initialRoster: KnowledgeItemRosterMember[];
}) {
  ...
  async function patchInvitees(body: { addUserIds?: string[]; removeUserIds?: string[] }): Promise<boolean> {
    ...
    try {
      ...
      const res = await fetch(`/api/library/${itemId}/invitees`, {
        method: "PATCH",
