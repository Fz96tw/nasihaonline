...
import type { ForumThreadRosterMember } from "@/lib/forums";
import type { DirectoryMember } from "@/lib/members";

/**
 * Author/admin-facing invited-list editor on a restricted standalone
 * thread's detail page (Member-Initiated Restricted Forum Threads,
 * §4.13/§11.16) — add and remove invitees after creation, each action
 * posted immediately (no separate "save" step). Mirrors
 * components/library/manage-invitees.tsx exactly, minus the RSVP-status
 * badge that doesn't apply here either.
 */
export function ManageThreadInvitees({
  threadId,
  initialRoster,
}: {
  threadId: string;
  initialRoster: ForumThreadRosterMember[];
}) {
  ...
  async function patchInvitees(body: { addUserIds?: string[]; removeUserIds?: string[] }): Promise<boolean> {
    ...
    try {
      ...
      const res = await fetch(`/api/forums/threads/${threadId}/invitees`, {
        method: "PATCH",
  ...
  async function addMember(member: DirectoryMember) {
    ...
    if (ok) {
      setRoster((current) => [...current, { userId: member.id, name: member.name, avatarUrl: member.avatarUrl }]);
    }
  }

  async function removeMember(userId: string, name: string | null) {
    if (roster.length <= 1) {
      setError("A restricted thread must keep at least one invited member.");
      return;
    }
    if (!window.confirm(`Remove ${name ?? "this member"} from the invited list? They'll lose access immediately.`)) return;
