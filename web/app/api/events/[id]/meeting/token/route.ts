 ...
 * route: access control lives inside getEventMeetingStatus, which only
 * allows a signed-out caller through for an `open` event. An anonymous
 * open-event attendee gets an ephemeral guest identity — LiveKit doesn't
 * need it to map to a real account, only to be unique per connection.
 */
