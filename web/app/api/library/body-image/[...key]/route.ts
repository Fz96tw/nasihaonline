...
/**
 * Streams a pasted Library blog_post body image from MinIO through our own
 * origin, gated by canViewPastedImage so a restricted item's embedded
 * image is never more visible than the item itself — same shape as
...
export async function GET(_request: Request, { params }: { params: { key: string[] } }) {
  ...
  if (!image || image.ownerType !== "library_item") {
