...
- `web/` — the Next.js 14 (App Router) application. TypeScript, Tailwind, shadcn/ui, Prisma + PostgreSQL, Clerk auth, Redis (rate limiting), Resend (email). Has its own `package.json`, `npm install`/`npm run dev`/`npm run build`/`npm run lint`. Local Postgres/Redis run via `homelab/docker-compose.yml`; Prisma migrations live in `web/prisma/migrations`. See `web/README.md` for setup (including required Clerk dashboard steps).
- `homelab/` and `vps/` — two separate, fully self-contained `docker-compose.yml` deployments (not variants of one shared file — see `docs/Nasiha_LiveKit_Hosting_Decision.md` for why). `homelab/` is this repo's actual working directory for local dev and the current AT&T-hosted deployment (`docker compose` commands run from inside `homelab/`, matching how `web/README.md` describes them). `vps/` is a reference copy of what's deployed on the Bluehost VPS (hand-synced via `scp`, not git-pulled on that host) — only `vps/` runs LiveKit (`livekit`/`egress` services) and `nginxproxymanager`; `homelab/`'s app container points its `LIVEKIT_URL` at that shared VPS instance rather than running its own.
